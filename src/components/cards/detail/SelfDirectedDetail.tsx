              ...
              {flow.map((step, i) => (
                <div key={i} className="flex gap-3">
                  <div className="flex flex-col items-center w-5 flex-shrink-0">
                    ...
                    {i < flow.length - 1 && (
                      <div className="w-px flex-1 bg-gray-100 mt-1" style={{ minHeight: 20 }} />
                    )}
                  </div>
                  ...
                  {onSaveDetails && (
                    <button
                      onClick={() => onSaveDetails("flow", flow.filter((_, idx) => idx !== i))}
...
function FlowItemEdit({ value, onSave }: { value: string; onSave: (v: string) => void }) {
